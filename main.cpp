...
constexpr std::array<int, 12> hand_chips = {5, 10, 20, 30, 30, 35, 40, 60, 100, 120, 140, 160};
constexpr std::array<int, 13> hand_mult = {1, 2, 2, 3, 4, 4, 4, 7, 8, 8, 12, 14, 16};
...
public:
    ...
    int hands_left;
    int discards_left;
    std::array<int, 9> ante_base_chips = {100, 300, 800, 2000, 5000, 11000, 20000, 35000, 50000};
    ...
    void play_hand(const std::vector<Card>& hand) {
        ...
        So, if you have a Vampire, this is where it will suck up the effects of the cards, 
        or if you have a Runner and a Straight gets detected, increases its chips.
        Same goes with other Jokers like Ride the Bus, Square Joker, Green Joker, etc.
        */
        /*
        2. Dealt hand scoring
        our dealt hand is analysed left to right,
        adding the chips, bonus effects, multiplier effects, triggering glass cards, etc.
        Specific order:
        - 1. Adding Base Card Chips

        - 2. Triggering Own Card Effects: +Chips (either from Bonus as +30 chips, or from Hiker), Mult card (+4 Mult), Lucky (Chances of +20 mult or $20).
        ...
        */
        ...
        auto max_it = std::max_element(ranked_hand.begin(), ranked_hand.end(), 
